...
function App() {
  ...
  const askQuestion = async () => {
    ...
    let response = await fetch(URL+"?key=AIzaSyCZR0onMRcO5xvTdewyJtvHZYl0BHi6DnY", {
      method: "POST",
  ...
  return (
    <div className={darkMode=='dark'?'dark':'light'}>
      <div className='grid grid-cols-5 text-center h-screen'>
        <select onChange={(e)=>setDarkMode(e.target.value)} className='fixed dark:text-white bottom-0 p-3'>
        ...
        <div className='col-span-4 p-10'>
          <h1 className='text-5xl bg-clip-text text-transparent bg-gradient-to-r from-pink-700 to-violet-700'>Hello user, Ask me Anything</h1>
          {
            loader ?
            <div role="status">
        ...
        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
          ...
          <div ref={scrollToAns} className='container h-180 overflow-scroll '>
          ...
          <div className='dark:bg-zinc-800 w-1/2 bg-red-100 dark:text-white text-zinc-800 m-auto p-1 pr-5 rounded-4xl border border-zinc-700 flex h-16'>
            <input type="text" className='w-full h-full p-3 outline-none' value={question}
              onKeyDown={isEnter}
              onChange={(e) => setQuestion(e.target.value)} placeholder='Ask me anythink' />
